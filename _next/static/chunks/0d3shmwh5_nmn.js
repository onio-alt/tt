...
/* Kodexpert: close the mobile navigation after client-side routing. */
(() => {
  const selector = "details.mobile-menu";
  const closeMenus = () => {
  ...
  document.addEventListener("click", (event) => {
    ...
    if (target.closest(`${selector} nav a`)) {
      closeMenus();
      return;
    }

    if (!target.closest(selector)) closeMenus();
  });
